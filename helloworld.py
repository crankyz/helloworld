...
class Configuration(object):
    def __init__(self, lr, us, ld, kp, bs, opt, video_dim, audio_dim, a_samples, n_epochs, n_hidden, n_layer, reg,
                 mask_thresh):
...
def train(model_selection, data_path, data_path_train, data_path_val, config, exp_num='0', tfrecord_mode='fixed'):
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope('train_batch'):
            train_data_manager = DataManager(config.audio_feat_dim, config.video_feat_dim, config.num_audio_samples,
                                             buffer_size=4000, mode=tfrecord_mode)
        ...
        with tf.name_scope('validation_batch'):
            val_data_manager = DataManager(config.audio_feat_dim, config.video_feat_dim, config.num_audio_samples,
                                           mode=tfrecord_mode)
        ...
        with tf.name_scope('placeholder'):
            ...
            input_mixed_specs = tf.placeholder(tf.float64, shape=[None, None, config.audio_feat_dim],
                                               name='input_mixed_specs')
        ...
        with tf.variable_scope('model'):
            if model_selection == 'vl2m':
                ...
                model = VL2MRef(input_video, sequence_lengths, tbm, mixed_sources, target_sources, input_mixed_specs,
                                keep_prob, config)
            elif model_selection == 'av_concat_mask':
                model = AudioVisualConcatMask(input_video, sequence_lengths, mixed_sources, target_sources,
                                              input_mixed_specs, keep_prob, config)
            elif model_selection == 'av_concat_mask_ref':
                model = AudioVisualConcatMaskRef(input_video, sequence_lengths, tbm, mixed_sources, target_sources,
                                                 input_mixed_specs, keep_prob, config)
            else:
                print(
                    'Model selection must be "vl2m", "vl2m_ref", "av_concat_mask" or "av_concat_mask_ref". Closing...')
                sys.exit()
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True,
            ...
            if last_checkpoint:
                ...
                trainingLogFile.write(
                    '## dropout keep probability (no dropout if 1): {:.6f}\n'.format(config.keep_prob))
            ...
            try:
                ...
                while True:
                    ...
                    mixed_audio_batch, base_paths_batch, other_paths_batch, mixed_paths_batch = sess.run(
                        next_train_batch)
                    ...
                    sdr_batch, sir_batch, sar_batch = sdr_batch_eval(base_audio_batch, other_audio_batch,
                                                                     enhanced_audio_batch,
                                                                     sequence_lengths=length_batch)
                    ...
                    if step % (n_steps_epoch // 16) == 0 or step == 1:
                        print(
                            'Step[{:7d}] Cost[{:3.5f}] LR[{:.6f}] L2-SPEC[{:3.5f}] SNR[{:3.5f}] SDR[{:3.5}] SIR[{:3.5}] SAR[{:3.5}]' \
                            .format(step, cost / config.batch_size, lr, l2_spec_batch.mean(), snr_batch.mean(),
                                    sdr_batch.mean(), sir_batch.mean(), sar_batch.mean()))
                    ...
                    if (step % n_steps_epoch == 0) and (step != 0):
                        print(
                            'Completed epoch {:d} at step {:d} --> Train cost: {:.6f} - L2-SPEC: {:.6f} - SNR: {:.6f} - SDR: {:.6f} - SIR: {:.6f} - SAR: {:.6f}' \
                        ...
                        val_cost, val_enhanced_audio, summaries = sess.run(
                            fetches=[model.loss, model.enhanced_sources, model.summaries],
                            feed_dict={
                        ...
                        val_sdr, val_sir, val_sar = sdr_batch_eval(val_base_audio, val_other_audio, val_enhanced_audio,
                                                                   sequence_lengths=val_length)
                        val_l2_spec = l2_batch_eval(val_base_audio, val_enhanced_audio, sequence_lengths=val_length)

                        print(
                            'Validation cost: {:.6f} - L2-SPEC: {:.6f} - SNR: {:.6f} - SDR: {:.6f} - SIR: {:.6f} - SAR: {:.6f}' \
                            .format(val_cost / num_examples_val, val_l2_spec.mean(), val_snr.mean(), val_sdr.mean(),
                                    val_sir.mean(), val_sar.mean()))
                        ...
                        trainingLogFile.write(
                            '{:d}\t[{:.6f}][{:.5f}|{:.5f}|{:.5f}|{:.5f}|{:.5f}|{:.5f}]\t[{:.5f}|{:.5f}|{:.5f}|{:.5f}|{:.5f}|{:.5f}]\n' \
                            .format(epoch_counter, lr, epoch_cost / num_examples_train, epoch_l2_spec.mean(),
                                    epoch_snr.mean(), epoch_sdr.mean(), epoch_sir.mean(), epoch_sar.mean(),
                                    val_cost / num_examples_val, val_l2_spec.mean(), val_snr.mean(), val_sdr.mean(),
                                    val_sir.mean(), val_sar.mean()))
                ...
                print('Done training for %d epochs, %d steps: validation cost = %.5f' % (
                config.num_epochs, step, val_cost / num_examples_val))
